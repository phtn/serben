...
import { progressBar, createBox, statusIndicator, sparkline, barChart } from '../ui/components.js';
import type { BenchmarkResult, ProgressCallback, MemoryBenchmarkOptions } from '../types/index.js';
...
export async function runMemoryBenchmark(
  ...
  console.log('\n');
  console.log(createBox('MEMORY BENCHMARK', [
    `${colors.muted('Total RAM:')} ${colors.neonCyan(formatBytes(totalMem))}`,
    `${colors.muted('Used:')} ${colors.neonOrange(formatBytes(usedMem))} ${colors.dimmed('(' + ((usedMem / totalMem) * 100).toFixed(1) + '%)')}`,
    `${colors.muted('Free:')} ${colors.neonGreen(formatBytes(freeMem))}`,
    `${colors.muted('Block Size:')} ${colors.neonPink(formatBytes(blockSize))}`,
    '',
    progressBar(usedMem, totalMem, 50, true, '#FF6B35', '#00FF88'),
    '',
    `${colors.dimmed('Running memory performance tests...')}`,
  ], 60, 'neonBlue'));
  console.log('\n');
  ...
  completedTests++;
  console.log(`${statusIndicator('passed')} ${colors.bright('Sequential Access')} ${progressBar(seqResult.score, 100, 30)}`);
  ...
  completedTests++;
  console.log(`${statusIndicator('passed')} ${colors.bright('Random Access')} ${progressBar(randResult.score, 100, 30)}`);
  ...
  completedTests++;
  console.log(`${statusIndicator('passed')} ${colors.bright('Allocation Stress')} ${progressBar(allocResult.score, 100, 30)}`);
  ...
  completedTests++;
  console.log(`${statusIndicator('passed')} ${colors.bright('Memory Bandwidth')} ${progressBar(bwResult.score, 100, 30)}`);
  ...
  completedTests++;
  console.log(`${statusIndicator('passed')} ${colors.bright('Memory Latency')} ${progressBar(latResult.score, 100, 30)}`);
  ...
  // Display summary
  console.log('\n');
  console.log(createBox('MEMORY BENCHMARK RESULTS', [
    '',
    ...
    `${colors.muted('Avg Throughput:')} ${colors.neonGreen(avgThroughput.toFixed(0))} ${colors.dimmed('ops/ms')}`,
    '',
...
export async function runMemoryStressTest(
  ...
  console.log('\n');
  console.log(createBox('MEMORY STRESS TEST', [
  ...
  console.log(`${colors.muted('   Average usage:')} ${colors.neonGreen((usageHistory.reduce((a, b) => a + b, 0) / usageHistory.length).toFixed(1) + '%')}`);
}
