...
import { progressBar, createBox, statusIndicator, barChart } from '../ui/components.js';
...
function sequentialWriteTest(filePath: string, totalSize: number, blockSize: number): { throughput: number; iops: number; duration: number } {
...
function sequentialReadTest(filePath: string, blockSize: number): { throughput: number; iops: number; duration: number } {
...
// Random write test
function randomWriteTest(basePath: string, fileCount: number, fileSize: number): { throughput: number; iops: number; duration: number } {
...
// Mixed I/O test
function mixedIOTest(basePath: string, iterations: number, fileSize: number): { throughput: number; iops: number; duration: number } {
  const data = generateRandomData(fileSize);
  let totalOps = 0;
  let totalBytes = 0;
...
export async function runStorageBenchmark(
  ...
  console.log('\n');
  console.log(createBox('STORAGE BENCHMARK', [
    `${colors.muted('Test Location:')} ${colors.neonCyan(TEMP_DIR)}`,
    `${colors.muted('File Size:')} ${colors.neonGreen(formatBytes(fileSize))}`,
    `${colors.muted('Block Size:')} ${colors.neonPink(formatBytes(blockSize))}`,
    `${colors.muted('Iterations:')} ${colors.neonOrange(String(iterations))}`,
    '',
    `${colors.dimmed('Running storage I/O performance tests...')}`,
  ], 60, 'neonGreen'));
  console.log('\n');
  ...
  try {
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Sequential Write')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonCyan(seqWriteResult.throughput.toFixed(1) + ' MB/s')}`);
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Sequential Read')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonCyan(seqReadResult.throughput.toFixed(1) + ' MB/s')}`);
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Random Write')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonGreen(randWriteResult.iops.toFixed(0) + ' IOPS')}`);
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Random Read')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonGreen(randReadResult.iops.toFixed(0) + ' IOPS')}`);
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Mixed I/O')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonOrange(mixedResult.iops.toFixed(0) + ' IOPS')}`);
    ...
    completedTests++;
    console.log(`${statusIndicator('passed')} ${colors.bright('Write Latency')} ${progressBar(results[results.length - 1].score, 100, 30)} ${colors.neonPink(latResult.avg.toFixed(2) + ' µs')}`);
  ...
  const avgThroughput = results.filter(r => r.throughputMBps > 0).reduce((sum, r) => sum + r.throughputMBps, 0) / results.filter(r => r.throughputMBps > 0).length;
  const avgIOPS = results.filter(r => r.iops > 0).reduce((sum, r) => sum + r.iops, 0) / results.filter(r => r.iops > 0).length;
  ...
  console.log(createBox('STORAGE BENCHMARK RESULTS', [
    '',
    ...
    `${colors.muted('Avg Throughput:')} ${colors.neonGreen(avgThroughput.toFixed(1))} ${colors.dimmed('MB/s')}`,
    `${colors.muted('Avg IOPS:')} ${colors.neonOrange(avgIOPS.toFixed(0))}`,
    '',
    `${colors.dimmed('─'.repeat(56))}`,
    '',
    ...results.map(r => 
      `  ${colors.muted(r.name.padEnd(20))} ${colors.neonCyan(r.score.toFixed(1).padStart(6))} ${colors.dimmed('pts')} ` +
      `${r.throughputMBps > 0 ? colors.neonGreen(r.throughputMBps.toFixed(1).padStart(8) + ' MB/s') : colors.neonGreen(r.iops.toFixed(0).padStart(8) + ' IOPS')}`
    ),
    '',
  ], 65, 'neonGreen'));
